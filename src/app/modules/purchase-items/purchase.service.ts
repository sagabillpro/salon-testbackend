...
const createBulk = async (
  ...
  try {
    ...
    const itemIds = data.purchaseLines.map((line) => line.service.id);
    ...
    data.purchaseLines = data.purchaseLines.map((line) => ({
      ...line,
      serviceId: latestItemMap[line.service.id],
    ...
    await dataSource.manager.transaction("SERIALIZABLE", async (manager) => {
      ...
      const stockEntries = data.purchaseLines.map((line) => {
        ...
        stock.serviceId = latestItemMap[line.service.id];
      ...
      const inventory = data.purchaseLines.map((line) => {
        ...
        invLine.serviceId = latestItemMap[line.service.id];
      ...
      await manager.save(ItemAvailable, itemAvailableEntry);
...
const editBulk = async (data: PurchaseHeaders) => {
  try {
    ...
    await dataSource.manager.transaction("SERIALIZABLE", async (manager) => {
      ...
      //create new purchase transaction
      const newPurchase = await createBulk(
        {
          ...data,
