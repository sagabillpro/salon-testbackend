...
const repository = async () => {
  ...
  //1. find all records
  const find = async (option?: FindManyOptions<CompanyCoupouns>) => {
    try {
  ...
  const findOne = async (option?: FindOneOptions<CompanyCoupouns>) => {
    try {
      return await repo.find(option);
    } catch (error) {
      throw error;
  ...
  const findOneById = async (id: number, filter?: FindOneOptions<CompanyCoupouns>) => {
    try {
  ...
  const createAll = async (data: CompanyCoupouns[]) => {
    try {
      const respo = repo.create(data);
      await repo.save(respo);
      return respo;
  ...
  //6. create multiple records
  const createBulk = async (data: CompanyCoupouns[]) => {
    try {
      const respo = repo.create(data);
      await repo.save(respo);
