...
const repository = async () => {
  ...
  //1. find all records
  const find = async (option?: FindManyOptions<Contact>) => {
    try {
  ...
  const findOne = async (option?: FindOneOptions<Contact>) => {
    try {
      return await repo.find(option);
    } catch (error) {
      throw error;
  ...
  const findOneById = async (id: number, filter?: FindOneOptions<Contact>) => {
    try {
  ...
  const createAll = async (data: Contact[]) => {
    try {
      const respo = repo.create(data);
      await repo.save(respo);
      return respo;
  ...
  //6. create multiple records
  const createBulk = async (data: Contact[]) => {
    try {
      const respo = repo.create(data);
      await repo.save(respo);
