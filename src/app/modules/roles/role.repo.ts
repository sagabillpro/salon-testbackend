...
const repository = async () => {
  ...
  //1. find all records
  const find = async (option?: FindManyOptions<Role>) => {
    try {
  ...
  const findOne = async (option?: FindOneOptions<Role>) => {
    try {
      return await repo.find(option);
    } catch (error) {
      throw error;
  ...
  const findOneById = async (id: number, filter?: FindOneOptions<Role>) => {
    try {
  ...
  const createAll = async (data: Role[]) => {
    try {
      const respo = repo.create(data);
      await repo.save(respo);
      return respo;
  ...
  //6. create multiple records
  const createBulk = async (data: Role[]) => {
    try {
      const respo = repo.create(data);
      await repo.save(respo);
