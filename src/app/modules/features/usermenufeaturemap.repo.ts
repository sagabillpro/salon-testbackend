...
const repository = async () => {
  ...
  const repo = dataSource.getRepository(UserMenusAndFeatures);
  //1. find all records
  const find = async (option?: FindManyOptions<UserMenusAndFeatures>) => {
    try {
  ...
  const findOne = async (option?: FindOneOptions<UserMenusAndFeatures>) => {
    try {
      return await repo.find(option);
    } catch (error) {
      throw error;
  ...
  const findOneById = async (id: number, filter?: FindOneOptions<UserMenusAndFeatures>) => {
    try {
  ...
  const createAll = async (data: UserMenusAndFeatures[]) => {
    try {
      const respo = repo.create(data);
      await repo.save(respo);
      return respo;
  ...
  //6. create multiple records
  const createBulk = async (data: UserMenusAndFeatures[]) => {
    try {
      const respo = repo.create(data);
      await repo.save(respo);
